...
class WorkflowBuilder extends DelegateSupport {
	...
	static workflow(String dsl) {
		...
		def delegate = new WorkflowDelegate(workflow)
		dslScript.metaClass = createEMC(dslScript.class, {
			ExpandoMetaClass emc ->
			emc.workflow = {map,Closure cl ->
				workflow.name  = map.name
				godeeper(cl, delegate)
			}
		})
		dslScript.run()
		if (!workflow.states.find{it.name == 'end'}) {
			delegate.end()
		}
		return workflow
	}
			...
			if (args?.length >=1) {
				...
				if (args[0] instanceof Map) {
					switch(args[0]?.type) {
						...
						break
						case CASE: "orfork"
							state = new RowkOrForkState()
						break
						case CASE: "orjoin"
							state = new RowkOrJoinState()
						break
						default:
							state = new RowkHumanState()
						break
					}
					closure = args[1]
				} else {
